...
export function AppointmentManager({
  ...
  const { doctors, currentUser, addAppointment } = useData();
  ...
  const handleGetRecommendation = async () => {
    ...
    if (!issue && !symptoms) {
      toast({
        ...
        description:
          'Please fill in your primary issue or symptoms to get a recommendation.',
      });
      return;
    }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setIsBooking(true);
    try {
        const selectedDoctor = doctors.find(d => d.id === parseInt(values.doctorId, 10));
        if(!selectedDoctor) {
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <Tabs defaultValue="book" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="view" onClick={fetchAppointments}>View Appointments</TabsTrigger>
          </TabsList>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="e.g., Allergies, Past Surgeries" {...field} />
                      </FormControl>
                    </FormItem>
                  )}
                />
                    ...
                    <FormItem>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" onClick={handleGetRecommendation} disabled={isRecommending}>
                            ...
                            <span className="ml-2 hidden sm:inline">Get AI Recommendation</span>
                        </Button>
                      </div>
                      <FormMessage />
                    </FormItem>
                  ...
                  {isLoadingAppointments ? (
                      ...
                      upcomingAppointments.map(appt => (
                          <div key={appt.id} className="p-4 border rounded-lg">
                              ...
                              <p className="text-sm mt-2">{format(new Date(appt.date), 'EEEE, MMMM d, yyyy')} at {appt.time}</p>
                          </div>
                      ))
                  ) : (
